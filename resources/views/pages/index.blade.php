...
@section('content')
    <section class="intro-section">
        ...
        {!! $personal->about_text !!}
        <a href="{{$personal->btn_contact_link ? $personal->btn_contact_link : 'javascript:void(0)'}}"
           class="btn btn-primary btn-hire-me">{{$personal->btn_contact_text}}</a>
    </section>
    <section class="resume-section">
        <div class="row">
            <div class="col-lg-6">
                ...
                <ul class="time-line">
                    @foreach($educationList as $key)
                        <li class="time-line-item">
                            <span class="badge badge-primary">{{$key->education_date}}</span>
                            <h6 class="time-line-item-title">{{$key->university_name}}</h6>
                            <p class="time-line-item-subtitle">{{$key->university_faculty}}</p>
                            <p class="time-line-item-subtitle">{{$key->university_degree}}</p>
                            <p class="time-line-item-content">{{$key->description}}</p>
                        </li>
                    @endforeach
                </ul>
            </div>
            <div class="col-lg-6">
                ...
                <ul class="time-line">
                    @foreach($experienceList as $key)
                        <li class="time-line-item">
                            <span class="badge badge-primary">{{$key->date}}</span>
                            <h6 class="time-line-item-title">{{$key->task_name}}</h6>
                            <p class="time-line-item-subtitle">{{$key->company_name}}</p>
                            <p class="time-line-item-content">{{$key->description}}</p>
                        </li>
                    @endforeach
                </ul>
